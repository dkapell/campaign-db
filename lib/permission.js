...
function check(req, permission, bypass){
    ...
    switch(permission){
        case 'site_admin':
            ...
            break;
        case 'admin':
            if (user.type === 'admin' && (bypass || (!req.session.gm_mode && !req.session.player_mode))){
                return true;
            }
            break;
        case 'gm':
            if (user.type.match(/^(admin|core staff)$/) && (bypass || !req.session.player_mode)){
                return true;
            }
            break;
        case 'contrib':
            if (user.type.match(/^(admin|core staff|contributing staff)$/) && (bypass || !req.session.player_mode)){
                return true;
            }
            break;
        case 'event':
            if (user.type.match(/^(admin|core staff|contributing staff|event staff)$/) && (bypass || !req.session.player_mode)){
                return true;
            }
            break;
        case 'player':
