        ...
        if (_.isArray(newSkill[field])){
            for (const skillId of newSkill[field]){
                if (_.indexOf(oldSkill[field], skillId) === -1){
                    ...
                    added.push(`${skill.name?skill.name:'TBD'} (${skill.source?skill.source.name:'No Source'})`);
                }
            }
        }
        if (_.isArray(oldSkill[field])){
            for (const skillId of oldSkill[field]){
                if (_.indexOf(newSkill[field], skillId) === -1){
                    const skill = await models.skill.get(skillId);
                    removed.push(`${skill.name?skill.name:'TBD'} (${skill.source?skill.source.name:'No Source'})`);
                }
            }
        }
...
exports.validate = async function validate(skill){
    const fields = {
        name: 'Name',
        cost: 'Cost',
        source_id: 'Source',
