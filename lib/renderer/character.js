...
module.exports = async function renderCharacter(character, options){
    ...
    const attributeList = await models.attribute.find({campaign_id: character.campaign_id});
    row = renderAttributes(row, attributeList);
    ...
    const skills = character.provides.skills
    ...
    function renderAttributes(row, attributeList){
        const attributeBoxHeight = 40;
        const attributes = _.keys(character.provides.attributes).sort((a, b) => {
            const attrA = _.findWhere(attributeList, {name:a});
            const attrB = _.findWhere(attributeList, {name:b});
            if (attrA){
                if (attrB){
        ...
        for (const attribute of attributes){
            addText(attribute, {font: 'Header Font', nowrap:true, align:'center'}, 14, offsetX, offsetY, attributeWidth - 10, 15);
            addText('' + character.provides.attributes[attribute], {font: 'Body Font Bold', nowrap:true, align:'center'}, 14, offsetX, offsetY + 15, attributeWidth - 10, 15);
            offsetX += attributeWidth;
        }

        const ritualAttributeHeight = 30/ritualAttributes.length;
        for (const attribute of ritualAttributes){
            addText(attribute, {font: 'Header Font', nowrap:true, align:'left'}, 14, offsetX, offsetY, attributeWidth*2 - 30, ritualAttributeHeight);

            const ritualAttributeX = offsetX + (attributeWidth*2 - (options.margin + (20*(ritualAttributes.length -1))));
            const ritualAttributeWidth = attributeWidth*2 - (ritualAttributeX - offsetX);

            addText('' + character.provides.attributes[attribute], {font: 'Body Font Bold', nowrap:true, align:'center'}, 14, ritualAttributeX, offsetY, ritualAttributeWidth, ritualAttributeHeight);
            offsetY += ritualAttributeHeight;
        }
