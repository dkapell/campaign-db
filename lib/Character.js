    ...
    async possibleSkills(all){
        ...
        await async.each(sources, async (source) => {
            ...
            for (const skill of sourceSkills){
                ...
                if (skill.id === 966){
                    console.log(skill.conflicts)
                    console.log(_.pluck(localSkills, 'id'));
                }

                if (_.isArray(skill.conflicts)){
                    for (const conflict of skill.conflicts){
                        if (skill.id === 966) { console.log('looking for ' + conflict); }
                        if (_.findWhere(localSkills, {id: Number(conflict)} )){
                            if (skill.id === 966) { console.log('found ' + conflict); }
                            continue;
                        }
                        if (skill.id === 966) { console.log('did not find ' + conflict); }
                    }
                }
