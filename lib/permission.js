...
function check(req, permission, bypass){
    ...
    switch(permission){
        case 'admin':
            if (user.user_type === 'admin' && (bypass || (!req.session.gm_mode && !req.session.player_mode))){
                return true;
            }
            break;
        case 'gm':
            if (user.user_type.match(/^(admin|core staff)$/) && (bypass || !req.session.player_mode)){
                return true;
            }
            break;
        case 'contrib':
            if (user.user_type.match(/^(admin|core staff|contributing staff)$/) && (bypass || !req.session.player_mode)){
                return true;
            }
            break;
        case 'event':
            if (user.user_type.match(/^(admin|core staff|contributing staff|event staff)$/) && (bypass || !req.session.player_mode)){
                return true;
            }
            break;
        case 'player':
