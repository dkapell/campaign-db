...
class Character{
    ...
    async init(){
        ...
        if (self.options.cloneId){
            await self.clone(self.options.cloneId, self.options.user_id, self.options.campaign_id);
    ...
    async update(data){
        ...
        if (_.has(data, 'custom_field')){
            const custom_fields = await models.custom_field.find({campaign_id:self._data.campaign_id, location:'character'});
            for (const field of custom_fields){
                const doc = {
                    character_id: self.id,
                    custom_field_id: field.id
    ...
    async addSource(sourceId, skipCost){
        ...
        const cp = await campaignHelper.cpCalculator(self._data.user_id, self._data.campaign_id);
        const campaign = await models.campaign.get(self._data.campaign_id);

        if (campaign.display_cp && user.type === 'player' && self._data.active && doc.cost > (cp.usable - self._data.cp)) {
            return;
        }
    ...
    async addSkill(skillId, details, skipCost){
        ...
        await cache.invalidate('character-skills-cache', self.id);
    ...
    async updateSkillDetails(characterSkillId, details){
        ...
        await models.character.update(self.id, self._data);
        return cache.invalidate('character-skills-cache', self.id);
    }
    ...
    async data(){
        ...
        doc.provides.attributes = await campaignHelper.attributeSorter(doc.provides.attributes, self._data.campaign_id);
        doc.provides.internalAttributes = await campaignHelper.attributeSorter(doc.provides.internalAttributes, self._data.campaign_id);
        ...
        doc.custom_field = await models.character_custom_field.find({character_id:self.id});
    ...
    async skills(noCache){
        const self = this;
        let skills = await cache.check('character-skills-cache', self.id);
        if (!noCache && skills){
            return skills;
        }

        skills = await models.character_skill.find({character_id:self.id});
        ...
        await cache.store('character-skills-cache', self.id, skills, 3);
        return skills;
    }
    ...
    async skillProvides(characterSkillId){
        const self = this;
        const skill = await self.skill(characterSkillId);
        return gatherProvides([skill], true, null);
    }

    async sources(skipSkills){
        const self = this;
    ...
    async recalculate(allSkills){
        ...
        const requiredSources = (await models.skill_source.find({campaign_id: self._data.campaign_id, required:true})).sort(skillHelper.sourceSorter);
        for (const source of requiredSources){
            await self.addSource(source.id, true);
        }
        // recalculate CP costs and required skills of Sources
        const charSources = _.groupBy(await self.sources(true), 'type_id');
    ...
    async possibleSkills(all){
        ...
        await async.each(sources, async (source) => {
            ...
            checkSkills: for (const skill of sourceSkills){
                ...
                if (campaign.display_cp && user.type === 'player' && self._data.active && Number(skill.next_cost) > (cp.usable - self._data.cp)) {
                    continue checkSkills;
                }
    ...
    async possibleSources(campaignId, all){
        ...
        return sources.filter (source => {
            ...
            if (campaign.display_cp && user.type === 'player' && self._data.active && source.cost > (cp.usable - self._data.cp)) {
                return false;
            }
            return true;
        });
    ...
    async activate(){
        ...
        if (user.type === 'player' && self._data.cp > cp.usable && campaign.display_cp) {
            return;
        }

        if (user.type === 'player'){
            const characters = await models.character.find({user_id: user.id, campaign_id:self._data.campaign_id});
...
async function gatherProvides (items, isSkills, provides, singleSkill){
    if (!provides){
        provides = {
            attributes: {},
            styles: {},
            ...
            if (_.isArray(data.custom_field)){
                ...
                for (const custom_field_data of data.custom_field){
                    ...
                    if (custom_field.display_to_pc){
                        output.custom_field.push(custom_field_data);
                    }
                }
            } else {
                output.custom_field = {};
