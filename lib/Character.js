...
class Character{
    ...
    async data(){
        ...
        doc.provides.attributes = prepAttributes(doc.provides.attributes, attributeList);
        /*const attributeList = await models.attribute.find({campaign_id:self._data.campaign_id});
        for (const attribute of attributeList){
            if (attribute.initial){
                if (_.has(doc.provides.attributes, attribute.name)){
                    doc.provides.attributes[attribute.name] += attribute.initial;
                } else {
                    doc.provides.attributes[attribute.name] = attribute.initial;
                }
            }
        ...
        //here
        const attributes = _.keys(doc.provides.attributes).sort((a, b) => {
        ...
        doc.provides.attributeList = [];
        for (const attribute of attributes){
            doc.provides.attributeList.push({
                name: attribute,
                value: doc.provides.attributes[attribute]
            });
        }*/
...
function prepAttributes(characterAttributes, attributeList){
    for (const attribute of attributeList){
        if (attribute.initial){
    ...
    //here
    const attributes = _.keys(characterAttributes).sort((a, b) => {
        const attrA = _.findWhere(attributeList, {name:a});
        const attrB = _.findWhere(attributeList, {name:b});
        if (attrA){
            if (attrB){
