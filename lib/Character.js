...
class Character{
    ...
    async create(data){
        ...
        if (_.has(data, 'foreordainment')){
            doc.foreordainment = data.foreordainment;
        }
        ...
        const sources = (await models.skill_source.find({required:true})).sort(skillHelper.sourceSorter);
    ...
    async clone(sourceCharacterId, userId){
        const self = this;
    ...
    async addSource(sourceId, skipCost){
        ...
        if (!(config.get('skill.showAll') || self.showAll || skill_source.display_to_pc)){
            return;
        }
        ...
        await models.character_skill_source.create(doc);

        const skills = (await models.skill.find({source_id: sourceId})).filter(skill => {
            return _.findWhere(skill.tags, {name: 'required'});
        });
        for (const skill of skills){
    ...
    async addSkill(skillId, details, skipCost){
        ...
        if (!(config.get('skill.showAll') || skill.status.purchasable)){
            return;
        }
        ...
        await models.character_skill.create(doc);
        if (skipCost){ return; }
        return self.calculateCP();
    }
    ...
    async removeSkill(characterSkillId, skipCost){
        ...
        if (source && _.findWhere(characterSkill.skill.tags, {name:'required'})){
    ...
    async data(){
        ...
        doc.user = await models.user.get(self._data.user_id);
        ...
        if (self._data.foreordainment){
            if (!_.has(doc.provides.traits, 'Character')){
                doc.provides.traits.Character = [];
            }
            doc.provides.traits.Character.unshift('Foreordained');
        }
    ...
    async skills(){
        ...
        return skills.map(skill => {
            ...
            doc.removable = true;
            if (_.findWhere(skill.skill.tags, {name: 'required'})){
                doc.removable = false;
            } else {
    ...
    async possibleSkills(all){
        ...
        await async.each(sources, async (source) => {
            ...
            for (const skill of sourceSkills){
                // Show all skills, if skill.showAll, otherwise show purchasable and visible skills (or purchasable + all)
                if (!(config.get('skill.showAll') || self.showAll || (skill.status.purchasable && (all || skill.status.display_to_pc)))){
                    continue;
                }
    ...
    async possibleSources(all){
        const self = this;
        const sources = await models.skill_source.find();
        ...
        return sources.filter (source => {
            ...
            if (!(config.get('skill.showAll') || self.showAll || all || source.display_to_pc)){
                return false;
            }
    ...
    async activate(){
        const self = this;
        const user = await models.user.get(self._data.user_id);
        self._data = await models.character.get(self.id);
...
async function gatherProvides (items, isSkills, provides, characterId){
    ...
    for (const item of items){
        ...
        if(isSkills && item.type && item.type.name === 'Centering'){
            provides.centering.push(item);
        }

        if(isSkills && item.type && item.type.name === 'Method'){
            provides.method = item;
        }
                ...
                switch (provider.type){
                    case 'stat': {
                        ...
                        if (provider.name && provider.name.match(/^\s*\[/)){
                            if (item.details && item.details.stat){
                                type = item.details.stat;
                            } else {
                        ...
                        if (!_.has(provides.stats, type)){
                            provides.stats[type] = 0;
                        }
                        provides.stats[type] += Number(provider.value);
                    ...
                    case 'crafting': {
                        ...
                        if (!_.has(provides.detrimentCards, type)){
                            const detrimentCard = await models.detriment_card.findOrClone(characterId, type);
                            detrimentCard.detriments.push({
                                content: config.get('detriments.resetText'),
                                uses:1
                            });
                            detrimentCard.name = `${capitalize(detrimentCard.type)} Detriment Card`;
                            provides.detrimentCards[type] = detrimentCard;
...
function formatUpdate(oldCharacter, newCharacter){
    const changes = [];
    for (const field of ['name', 'foreordainment']){
